...
namespace CizaCore
{
	public static class ObjectUtils
	{
		...
		public static T FindObject<T>(Predicate<T> predicate = null) where T : class =>
			Object.FindObjectsOfType<Object>().FirstOrDefault(obj => obj is T arg &&
		...
		public static List<T> FindObjects<T>(Predicate<T> predicate = null) where T : class =>
			Object.FindObjectsOfType<Object>().Where(obj => obj is T arg && (predicate == null || predicate(arg))).Cast<T>().ToList();
		...
		public static void AssertSingleInstance(this Object unityObject)
		{
			...
			Debug.Assert(Object.FindObjectsOfType(objectType).Length == 1, $"More than one instance of {objectType.Name} found on scene.");
		}
		...
		public static void AssertRequiredObjects(this Component component, params Object[] requiredObjects)
		{
			if (requiredObjects.Any(obj => !obj))
				throw new UnityException($"Unity object `{component}` is missing a required dependency. " + "Make sure all the required fields are assigned in the inspector and are pointing to valid objects.");
		}
